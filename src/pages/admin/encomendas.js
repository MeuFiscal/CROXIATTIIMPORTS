...
export async function renderAdminEncomendas(container) {
  if (!await requireAdmin()) return;

  return renderAdminLayout(container, 'Encomendas', async (content) => {
    content.innerHTML = `
      <div class="admin-table-wrap">
        <div class="admin-table-header" style="display:flex;justify-content:space-between;align-items:center;">
          <h2 style="font-size:1.2rem;font-weight:600;margin:0">Pedidos de Encomenda</h2>
          <input type="search" class="form-input" id="encomenda-search" placeholder="Buscar cliente..." style="max-width:220px;padding:8px 14px;font-size:.85rem" />
        </div>
        <div id="encomendas-table-wrap">
          <div style="padding:48px;text-align:center;color:var(--gray-400)">Carregando encomendas...</div>
        </div>
      </div>
    `;
    ...
    const load = async () => {
      const wrap = content.querySelector('#encomendas-table-wrap');
      ...
      if (!orders.length) {
        wrap.innerHTML = '<div class="admin-table-empty">Nenhuma encomenda pendente no momento</div>';
        return;
      }
            ...
            <tbody>
              ${orders.map(o => `
                <tr data-id="${o.id}">
                  ...
                  <td><span class="badge badge-warning">📦 Encomenda</span></td>
                  <td><span class="text-xs text-muted">${new Date(o.created_at).toLocaleDateString('pt-BR')}</span></td>
                  <td>
                    <div style="display:flex;gap:6px;flex-wrap:wrap">
                      <button class="btn btn-sm btn-success buy-btn" data-id="${o.id}">Pedido Comprado</button>
    ...
    // Search
    const searchInput = content.querySelector('#encomenda-search');
    let searchTimeout;
